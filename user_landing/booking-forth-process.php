<?php
session_start();
require '../includes/db.php'; // adjust path to your database connection
...
if (!isset($_SESSION['booking_data']['branch_id'])) {
    die("Branch ID is missing. Please go back and select a branch.");
}
$totalPrice = $_SESSION['booking_data']['totalPrice'];
$branch_id = $_SESSION['booking_data']['branch_id'];
$carId = $_SESSION['booking_data']['carId'];


// Fetch rules for the selected branch
$sqlRules = $pdo->prepare("SELECT rule_name FROM rules WHERE branch_id = :branch_id");
$sqlRules->execute(['branch_id' => $branch_id]);
$rules = $sqlRules->fetchAll(PDO::FETCH_ASSOC);
...
<body>
  ...
  <div class="sidebar">
    ...
    <a href="car-owner-shop-view.php?car_id=<?php echo $carId; ?>&branch_id=<?php echo $branch_id; ?>">
...
<!-- Form///////////// -->
          ...
          <form id="driverForm" method="POST" action="../booking_process/agree_rules.php">
            <div class="col-md-6 w-100">
                <div class="card h-100 p-4">
                    <div class="card-body">
                    
                      <div class="mb-4 d-flex align-items-center gap-2">
                        <img src="./image/rental-car-icon/booking-car-icon.png" alt="Driver Icon" style="width: 25px; height: 25px;">
                        <span style="font-size: 18px; font-weight: bold;">Rental Rules</span>
                      </div>
                      <?php $ruleNumber = 1; ?>
                        <?php foreach($rules as $rule): ?>
                          <div>
                            <!-- rental rules -->
                            <p><strong>Rule <?php echo $ruleNumber++; ?>:</strong> <?php echo htmlspecialchars($rule['rule_name']); ?></p>
                          </div>
                        <?php endforeach; ?>
                      <div class="form-check mt-3">
                        <input class="form-check-input" type="checkbox" id="agreeRules" name="agree" required>
                        <label class="form-check-label" for="agreeRules">
                          I agree to the rental rules
                        </label>
                      </div>
